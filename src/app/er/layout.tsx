      ...
      <div className="border-b border-[var(--grey-border)] bg-[var(--surface-card)] px-4 py-2 md:px-8">
        <div className="mx-auto flex max-w-[1600px] flex-wrap items-center justify-between gap-2 text-xs font-semibold text-[var(--blue)]/80">
          ...
          <nav className="flex flex-wrap items-center gap-2" aria-label="E&R segment">
            <Link href={erPaths.home(erLinkMode)} className="rounded-full px-2 py-1 hover:bg-[var(--icon-bg-muted)]">
              Segment home
            </Link>
            <span className="text-[var(--grey-border)]" aria-hidden>
              ·
            </span>
            <Link href={erPaths.operatorLens(erLinkMode)} className="rounded-full px-2 py-1 hover:bg-[var(--icon-bg-muted)]">
              Operator view
            </Link>
            <span className="text-[var(--grey-border)]" aria-hidden>
              ·
            ...
            <Link href={erPaths.personae(erLinkMode)} className="rounded-full px-2 py-1 hover:bg-[var(--icon-bg-muted)]">
              Personae
            </Link>
            <Link href={erPaths.journey(erLinkMode)} className="rounded-full px-2 py-1 hover:bg-[var(--icon-bg-muted)]">
              Journey
            </Link>
            <Link href={erPaths.needs(erLinkMode)} className="rounded-full px-2 py-1 hover:bg-[var(--icon-bg-muted)]">
              Needs
            </Link>
            <Link href={erPaths.ifm(erLinkMode)} className="rounded-full px-2 py-1 hover:bg-[var(--icon-bg-muted)]">
              IFM
            </Link>
          </nav>
        </div>
